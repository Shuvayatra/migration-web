...
<div class="form-group {{ $errors->has('metadata.deeplink') ? 'has-error' : ''}}">
	...
	<div class="col-sm-6">
		<select name="deeplink" class="form-control">
			<option value="">Select</option>
			<option {{(isset($notice->deeplink) && $notice->deeplink =="shuvayatra://home")?"selected":""}} value="shuvayatra://home">Home</option>

			<optgroup label="Destination">
				@foreach($countries as $key => $country)
					<option {{(isset($notice->deeplink) && $notice->deeplink =="shuvayatra://destination?destination_id=".$key)?"selected":""}} value="shuvayatra://destination?destination_id={{$key}}">{{$country}} </option>
				@endforeach
			</optgroup>
			<optgroup label="Dynamic">
				@foreach($dynamic_screens as $key => $dynamic_screen)
					<option {{(isset($notice->deeplink) && $notice->deeplink =="shuvayatra://screen?screen_id=".$key)?"selected":""}} value="shuvayatra://screen?screen_id={{$key}}">{{$dynamic_screen}}</option>
				@endforeach
			</optgroup>
			<optgroup label="Post">
				@foreach($posts as $post)
					<option {{(isset($notice->deeplink) && $notice->deeplink =="shuvayatra://".$post['metadata']->type."?post_id=".$post['id'])?"selected":""}} value="shuvayatra://{{$post['metadata']->type}}?post_id={{$post['id']}}">{{$post['metadata']->title}}</option>
				@endforeach
			</optgroup>

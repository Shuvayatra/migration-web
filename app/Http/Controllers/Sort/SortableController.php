...
class SortableController extends Controller
{
    ...
    public function sort(Request $request)
    {
        try {
            ...
            list($entityClass, $relation) = $this->getEntityInfo(
                $sortableEntities,
                (string) $request->input('entityName')
            );
    ...
    protected function getValidator($sortableEntities, $request)
    {
        ...
        if (!$relation) {
            ...
            $rules['id'] .= '|exists:'.$pivotTable.','.$relationObject->getOtherKey(
                ).','.$relationObject->getForeignKey().','.$request->input('parentId');
            $rules['positionEntityId'] .= '|exists:'.$pivotTable.','.$relationObject->getOtherKey(
                ).','.$relationObject->getForeignKey().','.$request->input('parentId');
        }
